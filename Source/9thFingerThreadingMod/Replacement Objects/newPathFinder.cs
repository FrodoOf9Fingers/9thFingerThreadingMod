...
namespace Verse.AI
{
    public class newPathFinder
	{
        ...
        internal class CostNodeComparer : IComparer<newPathFinder.CostNode>
        {
            public int Compare(newPathFinder.CostNode a, newPathFinder.CostNode b)
            {
        ...
        /// <summary>
        /// New PawnPathPool to superceed the pool used by the map.
        /// </summary>
        private PawnPathPool pathPool;

        private Map map;

        private FastPriorityQueue<newPathFinder.CostNode> openList;

        private newPathFinder.PathFinderNodeFast[] calcGrid;
        ...
        public newPathFinder(Map map)
        {
            ...
            this.calcGrid = new newPathFinder.PathFinderNodeFast[this.mapSizeX * this.mapSizeZ];
            this.openList = new FastPriorityQueue<newPathFinder.CostNode>(new newPathFinder.CostNodeComparer());
            this.regionCostCalculator = new RegionCostCalculatorWrapper(map);
            pathPool = new PawnPathPool(map);
        }
                ...
                switch (traverseParms.mode)
                {
                    case TraverseMode.ByPawn:
                        if (!traverseParms.canBash && building_Door.IsForbiddenToPass(pawn))
                        {
                            ...
                            {
                                newPathFinder.DebugFlash(b.Position, b.Map, 0.77f, "forbid");
                            }
                            return 2147483647;
                        }
                        if (!building_Door.FreePassage)
                        {
                            ...
                            if (DebugViewSettings.drawPaths)
                            {
                                newPathFinder.DebugFlash(b.Position, b.Map, 0.34f, "cant pass");
                            }
                            return 2147483647;
                        }
                        break;
        ...
        {
            return newPathFinder.BlocksDiagonalMovement(x, z, this.map);
        }

        private bool BlocksDiagonalMovement(int index)
        {
            return newPathFinder.BlocksDiagonalMovement(index, this.map);
        }

        public static bool BlocksDiagonalMovement(int x, int z, Map map)
        {
            return newPathFinder.BlocksDiagonalMovement(map.cellIndices.CellToIndex(x, z), map);
        }
        ...
        {
            newPathFinder.DebugFlash(c, this.map, colorPct, str);
        }
        ...
        private void InitStatusesAndPushStartNode(ref int curIndex, IntVec3 start)
        {
            ...
            this.openList.Clear();
            this.openList.Push(new newPathFinder.CostNode(curIndex, 0));
        }
        ...
        private float DetermineHeuristicStrength(Pawn pawn, IntVec3 start, LocalTargetInfo dest)
        {
            ...
            return (float)Mathf.RoundToInt(newPathFinder.NonRegionBasedHeuristicStrengthHuman_DistanceCurve.Evaluate(lengthHorizontal));
        }
