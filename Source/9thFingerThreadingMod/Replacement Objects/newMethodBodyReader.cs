...
{
    public class FixedMethodBodyReader
    {
        ...
        public static List<ILInstruction> GetInstructions(MethodBase method, ILGenerator generator)
        {
        ...
        public FixedMethodBodyReader(MethodBase method, ILGenerator generator)
        {
        ...
        {
            List<ILInstruction> insToRemove = new List<ILInstruction>();
            ...
            foreach (var instruction in insToRemove)
            {
                int offset = 0;
                foreach (var oldIns in ilInstructions)
                {
                    oldIns.offset -= offset;
                    if (oldIns.Equals(instruction))
                    {
                        offset = 2;
                    }
                }
                ilInstructions.Remove(instruction);
            }
        }

        // declare local variables
        public void DeclareVariables(LocalBuilder[] existingVariables = null)
        {
